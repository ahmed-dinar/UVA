...
int bfs(int x,int y,char ch,pri cur){
    queue<int>qx,qy;
    qx.push(x);
    qy.push(y);
    vis[x][y]=1;
    ...
    while(!qx.em){
        ...
        REP(i,4){
            ...
            if( valid(nx) && m[nx][ny]==ch && !vis[nx][ny] ){
                vis[nx][ny]=1;
                ret++;
                qx.push(nx);
                qy.push(ny);
...
int main(){
  ...
  //  fileout;
    ...
    int x,y;
    while( sc2(r,c) == 2 ){
        REP(i,r) REP(j,c) cin>>m[i][j],vis[i][j]=0;

        IO(x); IO(y);

        int ans=0;
        REP(i,r) REP(j,c) if(m[i][j]==m[x][y] && !vis[i][j]) ans=max(bfs(i,j,m[i][j],mp(x,y)),ans);
