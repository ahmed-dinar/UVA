...
void Permute(int k,char w[],int ln,int check[])
{
    if(k == ln){
        permutation[k] = '\0';
        if(repeat[permutation] != 0) return;

        printf("%s\n",permutation);
        repeat[permutation] = 1;
        return;
    }
    else{
