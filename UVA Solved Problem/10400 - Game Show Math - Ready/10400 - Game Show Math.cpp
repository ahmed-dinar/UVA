...
int a[102],h=32000,n,k,posibble;
int dp[102][64001];
char expression[] = {'*','+', '-', '/'};
int ans[101];

void solve(int i,int x,int e){

    if(posibble) return;

    if(i==n){
        if(x==k){
            REP(f,n-1) printf("%d%c",a[f],expression[ans[f]]);
            printf("%d=%d\n",a[n-1],k);
            posibble=1;
        }
        return;
    ...
    p=x*a[i];
    if(p<=h && p>=-h) ans[e]=0,solve(i+1,p,e+1);

    if(posibble) return;

    p=x+a[i];
    if(p<=h) ans[e]=1,solve(i+1,p,e+1);

    if(posibble) return;

    p=x-a[i];
    if(p>=-h) ans[e]=2,solve(i+1,p,e+1);
}
...
int main(){
    ...
    while(t--){
        ...
        REP(i,n){
            IO(a[i]);
            REP(j,64001) dp[i][j]=0;
        }
        IO(k);
        posibble=0;
        solve(1,a[0],0);
        if(!posibble) puts("NO EXPRESSION");
    }
    return 0;
}
