...
int bfs_Joe(int sr,int sc){
    queue<pr>q;
    q.push( mp(sr,sc) );
    joe[sr][sc]=0;
    while(!q.empty()){
        pr p = q.front(); q.pop();
        if( Exit(p.fi,p.ss) ) return joe[p.fi][p.ss]+1;
        REP(i,4){
            int rr=p.fi+x[i];
            int cc=p.ss+y[i];
            if( valid_move(rr,cc) && joe[p.fi][p.ss]+1<joe[rr][cc] ){
                if( fire[rr][cc]>joe[p.fi][p.ss]+1 ){
                    joe[rr][cc] = joe[p.fi][p.ss]+1;
                    q.push( mp(rr,cc) );
                }
            }
        }
