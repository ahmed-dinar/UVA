...
int isConnected(int u){
    ver=0;
    CL(vis);
    dfs(u);
    return (ver==vertex.sz);
}

int isEulerian(int u){

    for(set<int>::iterator it=vertex.begin(); it!=vertex.end(); it++){
        int v=*it;
        if( (g[v].sz&1) ) return 0;
    }

    if( !isConnected(u) ) return 0;

    return 1;
}

void printEulerCircuit(int u){
    REP(i,g[u].sz){
        int v=g[u][i];
        if(edge[u][v]>0){
            edge[u][v]--;
