...
void bin(int n){
    ...
    REP(i,12-p.sz) cout << "0";
    ROF(i,(int)p.sz-1) cout << p[i];
    nl;
}

void bitmask(int mask,int balls){
    ...
    REP(i,12){
        if( check(mask,i)!=0  ){
            if( i<9 && check(mask,i+1)!=0 && check(mask,i+2)==0 ){
                int m=mask;
                mask=off(mask,i);
                mask=on(mask,i+2);
                mask=off(mask,i+1);
                bitmask(mask,balls-1);
                mask=m;
            }
            if( i>1 && check(mask,i-1)!=0 && check(mask,i-2)==0 ){
                int m=mask;
                mask=off(mask,i);
                mask=on(mask,i-2);
                mask=off(mask,i-1);
...
int main(){
   ...
   // fileout;
    ...
    while(t--){
        ...
        cin>>s;
        int n=0,balls=0;
        ROF(i,(int)s.sz-1) if(s[i]=='o') n=on(n,11-i),balls++;
        MS(dp,-1);
        res=oo;
        bitmask(n,balls);
        printf("%d\n",res);
