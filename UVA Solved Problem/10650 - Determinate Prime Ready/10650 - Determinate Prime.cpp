...
vci prime,occur,ans[170];

void sieve(){
    int flag[MAX]={0};
    for(int i=3; i*i<=MAX; i+=2) if(!flag[i]) for(int j=i*i; j<MAX; j+=2*i) flag[j]=1;

    prime.pb(2);
    for(int i=3; i<MAX; i+=2) if(!flag[i]) prime.pb(i);
}

void preGen(){
    int pre=3,dis=3-2,tot=0;
    ans[tot].pb(2);
    ans[tot].pb(3);
    For(i,2,prime.sz){
        if( (prime[i]-pre)!=dis ){

            if(ans[tot].sz>=3) occur.pb(ans[tot][0]),tot++;
            else ans[tot].cl;
...
int main(){
    ...
    while( 1 ){
        ...
        if(n+m==0) break;
        if(n>m) swap(n,m);
        int x=lower_bound(all(occur),n)-occur.begin();
        while(1){
            int siz=ans[x].sz;
            if(siz==0 || ans[x][siz-1]>m) break;
            printf("%d",ans[x][0]);
            For(i,1,siz) printf(" %d",ans[x][i]);
            nl;
            x++;
        }
    }
