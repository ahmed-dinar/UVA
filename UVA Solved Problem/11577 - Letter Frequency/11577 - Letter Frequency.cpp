...
struct let
{
    char c;
    int freq;
    let(char c,int freq) : c(c),freq(freq){}
    bool operator<(const let& p)const {
        if(freq==p.freq) return c<p.c;
...
int main(){
   ...
   // fileout;
    ...
    while(t--){
        ...
        int ln=s.sz;
        REP(i,ln) if(isalpha(s[i])) s[i]=tolower(s[i]),f[s[i]]++;

        for(char ch='a'; ch<='z'; ch++) ans.pb(let(ch,f[ch]));

        sort(all(ans));

        REP(i,ans.sz){
            if(ans[i].freq!=ans[0].freq) break;
            printf("%c",ans[i].c);
        }
        nl;
        f.cl;
